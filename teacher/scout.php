 ...
 foreach (@$_FILES["att"]["error"] as $key => $error) {
  if ($error == UPLOAD_ERR_OK) {
   ...
   if (move_uploaded_file($tmp_name, "$folder/$newname")) {
    $sql = "INSERT INTO `tb_scout` VALUES (NULL,'$tid','$year','$newname','$filename')";
...
?>
<div>
    <form action="index.php?module=scout" id="addBook" enctype="multipart/form-data" method="POST">
        <div class="mb-1 col-md-8 mx-auto" id="attFile">
            <label for="att" class="form-label fw-bold">เพิ่มเอกสารทางลูกเสือ</label>
...
if (mysqli_num_rows($result) > 0) {
 ...
 while ($data = mysqli_fetch_array($result)) {
  ...
  echo "<a href='delfile.php?mode=scout&fid=$id' style='color:red' class='delbtn' data-bs-toggle='tooltip' data-bs-placement='top' title='ลบ'><i class='fas fa-times'></i></a>";
  echo "</li>";
  $i++;
 }
 echo "</ul>";
